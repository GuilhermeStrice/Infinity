...
namespace Infinity.Core
{
    public unsafe struct MessageReader
    {
        ...
        {
            throw new InfinityException("MessageReader must be created with a MemoryChunk or ChunkAllocator");
        }

        public MessageReader(MemoryChunk chunk)
        {
            _chunk = chunk;
            Position = 0;
        }

        public MessageReader(ChunkAllocator allocator)
        {
            _chunk = new MemoryChunk(allocator);
            Position = 0;
        }

        public MessageReader(ChunkAllocator allocator, byte[] buffer, int offset, int length)
        {
            if (buffer == null) throw new ArgumentNullException(nameof(buffer));
            if (offset < 0 || length < 0 || offset + length > buffer.Length)
                throw new ArgumentOutOfRangeException(nameof(offset), "Offset and length must be within buffer bounds.");

            _chunk = new MemoryChunk(allocator, length);
